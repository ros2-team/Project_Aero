...
class WebBridgeNode(Node):
    ...
    def plan_callback(self, msg):
        ...
        try:
            url = f"{self.flask_base_url}/api/navigation/plan"  
            headers = {"Content-Type": "application/json"}
            payload = {"plan": self.plan}
            
            res = requests.post(url, data=json.dumps(payload), headers=headers, timeout=1.0)
            if res.status_code == 200:
                self.get_logger().info(f"Flask에 plan 경로 전송 성공 (points: {len(self.plan)})")
            else:
                self.get_logger().error(f"plan 전송 실패 (HTTP {res.status_code})")
        except Exception as e:
            self.get_logger().error(f"plan 전송 중 예외 발생: {e}")
